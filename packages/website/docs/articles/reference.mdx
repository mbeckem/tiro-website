...
The optional update expression is evaluted after every completed loop iteration.
...
A field expression evalautes the expression left to the `.` symbol and yields the specified field of that value.
...
The `?[` can be used to make the value lookup optional.
If the left hand side is `null`, the index expression will also yield `null`.
