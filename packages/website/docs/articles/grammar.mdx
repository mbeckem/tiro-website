...
Whitespace consists of a sequence of characters having the Unicode _White_Space_ property.
Whitespace outside of tokens is not significant: its only use is to separate adjacent tokens that would otherwise be parsed as a single token.
