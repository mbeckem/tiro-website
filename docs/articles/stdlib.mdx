...
```
func failure(reason: value): Reason;
```
...
From within the `panic` function, a new [Exception](#type-exception) is constructed with the given `message` and [function unwinding](https://en.wikipedia.org/wiki/Call_stack#Unwinding) is started.
...
```
func catch_panic(func: Function): Result;
```

Invokes `func()` and stops propagation of any non-fatal panic thrown by `func` or its callees.
After execution of `func()` completes (either through panic or through normal return), a [Result](#type-result) is returned:

- If `func` returned normally, its return value is wrapped into a success result and returned.
